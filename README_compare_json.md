...
1. **Computed** — ключи вида `"<версия>-<длинное_имя_издания>"`. Пересчитываются с нуля каждый прогон. Ячейка `(версия, издание)` ставится в `true`, если выполнено **хотя бы одно** из условий:
   - `feature.tech` присутствует в этой ячейке `conf.json`/`contrib.json` (патч, утилита или contrib);
   - в исходном `feature.matrix` стоит `"pg-<версия>": true` (см. раскрытие ниже).

   Если ни одно условие не сработало — ключа просто нет (`false` не пишем).
2. **Preserved** — всё остальное, что было в `feature.matrix` исходного `descriptions.json`. Прежде всего это сами маркеры `pg-NN` («уже доступно в апстримном PostgreSQL N»), но это может быть и любой ваш ручной ключ. Эти ключи скрипт **никогда не трогает**: ни не удаляет, ни не перезаписывает.

### Раскрытие `pg-NN: true`

`pg-NN: true` означает «фича уже в апстримном PostgreSQL NN», то есть автоматически присутствует **во всех** изданиях Tantor NN. Если NN — это версия из `config.toml`, то для каждого её издания скрипт принудительно ставит `"<NN>-<edition>": true` (даже если самого `tech` в `conf.json` нет — например, потому что патч вошёл в апстрим и в Tantor его как отдельной записи больше нет).

Сам `pg-NN` ключ при этом сохраняется как preserved — раскрытие не подменяет его, а **дополняет**.

Если `pg-NN` указывает на версию, которой в `config.toml` нет (например, `pg-19` в задел на будущее) — ключ просто сохраняется как preserved, никаких ячеек не достраивается.
